...
// MARK: - Thread-safe Logging Queue

private let logQueue = DispatchQueue(label: "com.timeannouncer.log")

// MARK: - CoreAudio Helpers

/// Get the default output audio device ID
func caGetDefaultOutputDevice() -> AudioDeviceID {
...
func caGetDeviceName(_ deviceID: AudioDeviceID) -> String {
    ...
    return err == noErr ? (nameRef as String? ?? "unknown") : "unknown"
}

/// Get the transport type of a device (built-in, USB, Bluetooth, Virtual, etc.)
func caGetTransportType(_ deviceID: AudioDeviceID) -> UInt32 {
...
/// Get output volume (0.0–1.0) for a device. Returns -1 if not supported (e.g., some Bluetooth).
func caGetDeviceVolume(_ deviceID: AudioDeviceID) -> Float {
    ...
    return err == noErr ? volume : -1
}

/// Check if output is muted
func caIsOutputMuted(_ deviceID: AudioDeviceID) -> Bool {
    var muted: UInt32 = 0
    var size = UInt32(MemoryLayout<UInt32>.size)
    var address = AudioObjectPropertyAddress(
...
/// Find built-in speakers device ID (transport type == kAudioDeviceTransportTypeBuiltIn with output channels)
func caFindBuiltInSpeakers() -> AudioDeviceID? {
    var size: UInt32 = 0
    var address = AudioObjectPropertyAddress(
    ...
    for device in devices {
        let transport = caGetTransportType(device)
        if transport == kAudioDeviceTransportTypeBuiltIn {
            ...
            AudioObjectGetPropertyDataSize(device, &outAddr, 0, nil, &streamSize)
...
func caSetDefaultOutputDevice(_ deviceID: AudioDeviceID) -> Bool {
    var id = deviceID
    ...
    let err = AudioObjectSetPropertyData(AudioObjectID(kAudioObjectSystemObject), &address, 0, nil, size, &id)
    return err == noErr
}
...
class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, AVSpeechSynthesizerDelegate, NSPopoverDelegate {
    ...
    func applicationShouldHandleReopen(_ sender: NSApplication, hasVisibleWindows flag: Bool) -> Bool {
        if !flag || displayMode == .floatingWindow {
            switch displayMode {
            case .menuBar:
            ...
            case .floatingWindow:
                // Floating panel is always visible but doesn't count as "visible window" for reopen
                // Show the main settings window when user clicks Dock icon
                openSettingsWindow()
            }
        }
        return true
    ...
    func loadPreferences() {
        ...
        chimeVolume = defaults.object(forKey: "TAChimeVolume") as? Float ?? 2.5
        allDayEnabled = defaults.object(forKey: "TAAllDay") as? Bool ?? true

        if let savedVolume = defaults.object(forKey: "TAVolume") as? Int {
            speechVolume = Float(savedVolume) / 100.0
        }
    ...
    func buildSettingsTab(_ view: NSView) {
        ...
        chimeLbl.stringValue = "Alert chime"

        chimeVolumeLabel = makeLabel(in: voicev, x: boxW - 60, y: voiceBoxH - 52, width: 46, height: 18, fontSize: 12)
        chimeVolumeLabel.alignment = .right
        let chimePct = Int(chimeVolume / 3.0 * 100)
        chimeVolumeLabel.stringValue = chimePct == 0 ? "Off" : "\(chimePct)%"

        chimeVolumeSlider = NSSlider(frame: NSRect(x: 14, y: voiceBoxH - 70, width: boxW - 28, height: 20))
        chimeVolumeSlider.minValue = 0
        chimeVolumeSlider.maxValue = 100
        chimeVolumeSlider.integerValue = chimePct
        chimeVolumeSlider.target = self
        chimeVolumeSlider.action = #selector(chimeVolumeChanged(_:))
        chimeVolumeSlider.isContinuous = true
        voicev.addSubview(chimeVolumeSlider)
    ...
    var floatingTrackingArea: NSTrackingArea?

    // Custom view that tracks mouse hover for the floating panel
    class FloatingContentView: NSView {
        weak var appDelegate: AppDelegate?

        override func mouseEntered(with event: NSEvent) {
            appDelegate?.floatingMouseEntered()
        }
    ...
    func floatingMouseEntered() {
        ...
        // Show the "next" label on hover
        floatingNextLabel?.isHidden = false
    }

    func floatingMouseExited() {
        NSAnimationContext.runAnimationGroup { ctx in
            ctx.duration = 0.4
            floatingPanel?.animator().alphaValue = 0.35
        }
    ...
    func setupFloatingMode() {
        ...
        floatingPanel?.alphaValue = 0.35  // Transparent when idle

        // Custom content view with hover tracking
        let contentView = FloatingContentView(frame: NSRect(x: 0, y: 0, width: panelW, height: panelH))
        contentView.appDelegate = self
        contentView.wantsLayer = true
        contentView.layer?.cornerRadius = 14
        contentView.layer?.masksToBounds = true

        // Dark translucent background with vibrancy
        ...
        // Status dot — small, left side
        floatingStatusDot = NSTextField(frame: NSRect(x: 10, y: (panelH - 16) / 2 + 4, width: 14, height: 16))
        floatingStatusDot?.isEditable = false; floatingStatusDot?.isBordered = false; floatingStatusDot?.drawsBackground = false
        floatingStatusDot?.font = NSFont.systemFont(ofSize: 10)
        contentView.addSubview(floatingStatusDot!)

        // Time label — clean monospace
        floatingTimeLabel = NSTextField(frame: NSRect(x: 24, y: (panelH - 26) / 2 + 4, width: panelW - 34, height: 26))
        floatingTimeLabel?.isEditable = false; floatingTimeLabel?.isBordered = false; floatingTimeLabel?.drawsBackground = false
        floatingTimeLabel?.font = NSFont.monospacedDigitSystemFont(ofSize: 18, weight: .medium)
        floatingTimeLabel?.textColor = .white
        contentView.addSubview(floatingTimeLabel!)

        // Next announcement label — hidden by default, shown on hover
        floatingNextLabel = NSTextField(frame: NSRect(x: 10, y: 4, width: panelW - 20, height: 14))
        floatingNextLabel?.isEditable = false; floatingNextLabel?.isBordered = false; floatingNextLabel?.drawsBackground = false
        floatingNextLabel?.font = NSFont.systemFont(ofSize: 9, weight: .light)
        floatingNextLabel?.textColor = NSColor.white.withAlphaComponent(0.6)
        floatingNextLabel?.isHidden = true  // Only visible on hover
        contentView.addSubview(floatingNextLabel!)
        ...
        floatingTrackingArea = trackingArea
    ...
    private static let floatingTimeFormatter: DateFormatter = {
    ...
    func refreshUI() {
        ...
        let secsUntilNext = max(0, (announcementInterval - (minute % announcementInterval)) * 60 - second)
        ...
        if let fp = floatingPanel, fp.isVisible {
            floatingTimeLabel?.stringValue = Self.floatingTimeFormatter.string(from: Date())
    ...
    private func getAudioState() -> (vol: Int, muted: Bool, device: String, isVirtual: Bool) {
        ...
        let name = caGetDeviceName(deviceID)
        let rawVol = caGetDeviceVolume(deviceID)  // 0.0–1.0 or -1 if unsupported
        let vol = rawVol >= 0 ? Int(rawVol * 100) : -1
        let muted = caIsOutputMuted(deviceID)
        let transport = caGetTransportType(deviceID)
        let isVirtual = transport == kAudioDeviceTransportTypeVirtual
        return (vol, muted, name, isVirtual)
    }

    /// Force audio to built-in speakers if a virtual device has stolen the output (uses CoreAudio, no external tools)
    private func ensureBuiltInSpeakers(_ isVirtual: Bool, currentDevice: String) {
        guard isVirtual else { return }
        if let builtIn = caFindBuiltInSpeakers() {
            let builtInName = caGetDeviceName(builtIn)
            logEvent("AUDIO_DEVICE_FIX switching from \"\(currentDevice)\" to \"\(builtInName)\"")
            if !caSetDefaultOutputDevice(builtIn) {
                logEvent("AUDIO_DEVICE_FIX failed to set default output device")
            }
        } else {
            logEvent("AUDIO_DEVICE_FIX no built-in speakers found — cannot switch from \"\(currentDevice)\"")
        }
    }

    /// Speech pipeline v4.3 — fully off main thread, CoreAudio state, NSSound chime, lid-aware.
    private func speak(_ text: String) {
        // Kill any in-progress speech (main thread — fast)
        if synthesizer.isSpeaking { synthesizer.stopSpeaking(at: .immediate) }
        if let prev = sayProcess, prev.isRunning { prev.terminate() }
        sayProcess = nil

        // Capture values needed by the background block
        let voiceName = selectedVoiceName
        let appVol = speechVolume
        let chimeVol = chimeVolume

        // Everything else runs on the serial speech queue — zero main thread blocking
        speechQueue.async { [weak self] in
            guard let self = self else { return }

            // ── 1. Audio state via CoreAudio (< 1ms) ──
            let audio = self.getAudioState()
            let audioTag = "sysVol=\(audio.vol)% muted=\(audio.muted) device=\"\(audio.device)\" virtual=\(audio.isVirtual)"

            // ── 2. Fix volume if too low (osascript — only for SET, not query) ──
            if audio.muted || (audio.vol >= 0 && audio.vol < 60) {
            ...
            // ── 3. Force built-in speakers if virtual device ──
            self.ensureBuiltInSpeakers(audio.isVirtual, currentDevice: audio.device)

            // ── 4. Lid check before chime ──
            if isLidClosed() { return }

            // ── 5. Pre-speech chime via NSSound (non-blocking) ──
            if chimeVol > 0 {
                DispatchQueue.main.sync {
                    if let chime = NSSound(contentsOfFile: "/System/Library/Sounds/Glass.aiff", byReference: true) {
                        chime.volume = min(1.0, chimeVol / 3.0)  // Map 0–3.0 afplay scale → 0–1.0 NSSound scale
                        chime.play()
                    }
                }
                Thread.sleep(forTimeInterval: 0.4)  // Gap between chime and speech
            }

            // ── 6. Lid check before render ──
            if isLidClosed() { return }

            // ── 7. Render speech to AIFF file ──
            let tmpFile = NSTemporaryDirectory() + "timeannouncer_speech.aiff"
            let sayProc = Process()
            sayProc.executableURL = URL(fileURLWithPath: "/usr/bin/say")
            var sayArgs = [String]()
            if let vn = voiceName { sayArgs.append(contentsOf: ["-v", vn]) }
            ...
            do {
                try sayProc.run()
                DispatchQueue.main.async { self.sayProcess = sayProc }
                logEvent("SPEECH_RENDER text=\"\(text)\" voice=\(voiceName ?? "default") \(audioTag)")
            } catch {
            ...
            guard sayProc.terminationStatus == 0 else {
                ...
                logEvent("SPEECH_RENDER_FAIL text=\"\(text)\" exit=\(sayProc.terminationStatus) err=\"\(errStr)\"")
                return
            }

            // ── 8. Verify rendered file ──
            ...
            // ── 9. Lid check before playback ──
            if isLidClosed() {
                try? FileManager.default.removeItem(atPath: tmpFile)
                return
            }

            // ── 10. Play with volume amplification ──
            let sysVol = max(audio.vol, 60)
            let comp = min(4.0, max(1.5, 100.0 / Float(sysVol)))
            let afplayVol = appVol * comp
            ...
            do {
                let playStart = Date()
                try playProc.run()
                DispatchQueue.main.async { self.sayProcess = playProc }
                logEvent("SPEECH_PLAY text=\"\(text)\" size=\(fileSize)b afplayVol=\(String(format: "%.1f", afplayVol))x comp=\(String(format: "%.1f", comp))x")

                // Check lid one more time and kill if closed
                if isLidClosed() { playProc.terminate() }

                playProc.waitUntilExit()
                let dur = Date().timeIntervalSince(playStart)
                let status = playProc.terminationStatus
                if status == 0 {
                    let warn = dur < 0.3 ? " SUSPECT_SHORT" : ""
                    logEvent("SPEECH_DONE text=\"\(text)\" dur=\(String(format: "%.1f", dur))s\(warn)")
                } else {
                    let pErr = String(data: playErr.fileHandleForReading.readDataToEndOfFile(), encoding: .utf8) ?? ""
                    logEvent("SPEECH_PLAY_FAIL text=\"\(text)\" exit=\(status) err=\"\(pErr.trimmingCharacters(in: .whitespacesAndNewlines))\"")
                }
            } catch {
    ...
    @objc func muteAction() {
        ...
        muteFor(minutes: durations[mutePopup.indexOfSelectedItem])
    }
    ...
    @objc func chimeVolumeChanged(_ sender: NSSlider) {
        // Slider 0-100 maps to afplay -v 0.0-3.0
        chimeVolume = Float(sender.integerValue) / 100.0 * 3.0
        let pct = sender.integerValue
        chimeVolumeLabel.stringValue = pct == 0 ? "Off" : "\(pct)%"
        savePreference("TAChimeVolume", value: chimeVolume)
    }
